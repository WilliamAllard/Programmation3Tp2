...
void TP2::lireLabyrinthe(const std::string filePath)
{
	...
	if (tp2.is_open()) {
        ...
        // Si il y a une solution, il ajout au struct TP2EtapeChemin les donn�es du fichier
...
{
    if (m_etapeChemin.empty()) throw std::runtime_error("Aucune solution � �crire");
...
void TP2::solution() {
    ...
    // Vecteur pour mettre les case pr�c�dent. Le -1 permet de dire qu'on est au d�but (Pratique pour le for)
    ...
    // Inverse le m_etapeChemin pour pemettre le chemin du debut a la fin et non de la fin au d�but
